...
def unitary_roberta():
    ...
    global PIPE
    if PIPE is None:
        PIPE = pipeline("text-classification", model="unitary/unbiased-toxic-roberta", device=0, return_all_scores=True)
    return PIPE
...
def main(args):
    ...
    cur_doc_idx = 0
    try:
        for i, out in enumerate(tqdm(unitary_roberta()(dataset, batch_size=32, truncation=True, max_length=512), total=len(dataset))):
            if cur_doc_idx != dataset.data[i]["doc_idx"]:
                cur_doc_idx = dataset.data[i]["doc_idx"]
                results1.append({"detoxify_scores" : json.dumps(cur_doc)})
                cur_doc = defaultdict(dict)
    ...
    with open(os.path.join(base_dir, "detoxify_scores.csv"), "w") as scores_f:
